...
runReport = function(pathRoot, seed) {
  ## read in true values
  parm.list = list()
  f = file(paste(pathRoot, "input/simulation_settings.txt", sep=""), "r")
  while (TRUE) {
    l = readLines(f, n=1)
    if (length(l)==0) {
      break
  ...
  wardLog = read.csv(file = paste(pathRoot, "input/wardLog.csv", sep=""))
  geneticDist = read.table(file=paste(pathRoot, "input/simDistances.txt", sep=""))
  ...
  transSummaryFile = paste(pathRoot, "inference/", seed, "/trans_summary.csv", sep="")
...
pathRoot = "/Users/davideyre/Drive/academic/research/transmission_modelling/cdiff_transmission_inference/xcode_project/sim_data/5_scenarios/simulation_911145/"
